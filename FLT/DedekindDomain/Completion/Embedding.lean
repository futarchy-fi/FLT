...
/-- A completion embedding has image generated by the image of the global field. -/
theorem NumberField.fieldRange_eq_adjoin_of_completion_embedding
    {K : Type*} [Field K] [NumberField K]
    (v : IsDedekindDomain.HeightOneSpectrum (𝓞 K))
    {L : Type*} [Field L] [NumberField L] [Algebra K L] [FiniteDimensional K L]
    {Ω : Type*} [Field Ω] [Algebra (v.adicCompletion K) Ω]
    ...
    g.fieldRange = IntermediateField.adjoin (v.adicCompletion K) (Set.range j) := by
  classical
  apply le_antisymm
  · rintro _ ⟨y, rfl⟩
    have hs : Function.Surjective
        ((Pi.evalAlgHom (v.adicCompletion K) _ w).comp
          (adicCompletion.baseChangeRight K L (𝓞 L) v)) :=
      (show Function.Surjective (Pi.evalAlgHom (v.adicCompletion K)
        (fun w : v.Extension (𝓞 L) ↦ w.1.adicCompletion L) w) from
          fun y ↦ ⟨Pi.single w y, by simp⟩).comp
        (adicCompletion.baseChangeRight_surjective K L (𝓞 L) v)
    obtain ⟨z, rfl⟩ := hs y
    induction z with
    | add a b ha hb => simpa only [map_add] using add_mem ha hb
    | tmul x y =>
      change g (algebraMap L _ x * algebraMap (v.adicCompletion K) _ y) ∈ _
      rw [map_mul, hg, g.commutes]
      exact mul_mem (IntermediateField.subset_adjoin _ _ ⟨x, rfl⟩)
