...
-- `projects/Chebotarev/CebotarevDensity/NumberFieldEulerProduct.lean`.
-- Upstream carries no per-file header; the repo-wide Apache-2.0 attribution is
-- reproduced here so the vendored copy is self-describing.
module
...
private lemma mul_sup_span_natCast_left {m n : ℕ} (hcop : Nat.Coprime m n)
  ...
  -- Vendor fix (import-pin drift): in FLT's pinned Mathlib `Ideal.mul_le_left : I * J ≤ I`
  -- and `Ideal.mul_le_right : I * J ≤ J`; upstream AINTLIB's pin had the two names
  -- swapped, so the two uses below are exchanged relative to the original file.
