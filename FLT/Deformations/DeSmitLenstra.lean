...
## Construction supplied elsewhere

This file supplies the unconditional reduction to a universal lift.  The explicit construction
for an absolutely irreducible `ρ` over a profinite group is completed in
`FLT.Deformations.DeSmitLenstra.UniversalTraceLift`: framed lifts are represented by the
profinite framed ring and then descended to the closed subring generated by traces using absolute
irreducibility (Propositions 2.5 and 2.6).
-/
