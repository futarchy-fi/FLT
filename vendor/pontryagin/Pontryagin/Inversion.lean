...
public import Pontryagin.Bochner
...
theorem _root_.PontryaginDual.integrable_char (σ : Measure (PontryaginDual G))
  ...
  (integrable_const (1 : ℝ)).mono' (continuous_char_apply x).aestronglyMeasurable
    (Eventually.of_forall fun χ => (Circle.norm_coe (χ x)).le)
...
theorem integral_char_mul_fourierTransform_cc
...
theorem continuous_integral_c0_mul (σ : Measure (PontryaginDual G))
...
theorem integral_fourier_mul_density (σ : Measure (PontryaginDual G))
...
theorem setLIntegral_re_fourierTransform_compact_le (hfi : Integrable f μ)
...
theorem measure_lt_re_fourierTransform_le (hfi : Integrable f μ)
