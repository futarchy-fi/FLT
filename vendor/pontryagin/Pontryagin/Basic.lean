...
discrete group `A`. -/
def ofAddChar (f : AddChar (Additive A) Circle) : PontryaginDual A where
  toFun a := f (.ofMul a)
...
@[simp]
lemma ofAddChar_apply (f : AddChar (Additive A) Circle) (a : A) :
...
def equivAddChar : PontryaginDual A ≃ AddChar (Additive A) Circle where
  toFun f :=
...
theorem eval_injective [Finite A] :
...
theorem eval_bijective [Finite A] :
