...
public import Pontryagin.Convolution
...
theorem IsPositiveType.sum_two {f : G → ℂ} (hf : IsPositiveType f) (x : G)
    (c₁ c₂ : ℂ) :
...
theorem sum_sum_char_eq (χ : PontryaginDual G) {n : ℕ} (x : Fin n → G)
    (c : Fin n → ℂ) :
...
theorem isPositiveType_integral_char {mΓ : MeasurableSpace (PontryaginDual G)}
  ...
  have hint : ∀ z : G, Integrable (fun χ : PontryaginDual G => (χ z : ℂ)) σ := fun z => by
    ...
    exact Eventually.of_forall fun χ => (Circle.norm_coe (χ z)).le
...
nonnegative. -/
theorem complex_nonneg_of_forall_dist_le {z : ℂ}
...
theorem exists_nhds_one_kernel_estimate {φ f : G → ℂ} (hφc : Continuous φ)
