...
Modified for Lean/Mathlib v4.34.0-rc1 by the FLT contributors.
-/
module

public import Pontryagin.PositiveType
public import Pontryagin.UnitizationSpectrum
public import Pontryagin.FourierDense
...
omit hφ hφc in
theorem ccFourierSubmodule_mem_def (u : ccFourierSubmodule μ) :
...
def bochnerVal (u : ccFourierSubmodule μ) : ℂ :=
  posPairing μ hφ hφc (toLpCc μ (bochnerRep μ u) (bochnerRep_continuous μ u)
    (bochnerRep_hasCompactSupport μ u))

theorem bochnerVal_eq (u : ccFourierSubmodule μ) {f : G → ℂ}
    (hfc : Continuous f) (hfs : HasCompactSupport f)
...
theorem bochnerVal_add (u v : ccFourierSubmodule μ) :
...
theorem bochnerVal_smul (c : ℂ) (u : ccFourierSubmodule μ) :
...
theorem norm_bochnerVal_le (u : ccFourierSubmodule μ) :
...
def bochnerSubCLM : ccFourierSubmodule μ →L[ℂ] ℂ :=
...
theorem bochnerCLM_coe (u : ccFourierSubmodule μ) :
...
theorem continuous_integral_char (σ : Measure (PontryaginDual G)) [IsFiniteMeasure σ]
  ...
  have hint : ∀ z : G, Integrable (fun χ : PontryaginDual G => (χ z : ℂ)) σ := fun z =>
    (integrable_const (1 : ℝ)).mono' (hcont z).aestronglyMeasurable
      (Eventually.of_forall fun χ => (Circle.norm_coe (χ z)).le)
...
theorem bochnerCLM_realCc (γ : C_c(PontryaginDual G, ℝ)) :
