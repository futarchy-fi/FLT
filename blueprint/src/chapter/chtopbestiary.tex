...
$v\in S$ we are given a nonempty, $\Gal(L_v/K_v)$-invariant, open subset $\Omega_v\subseteq T(L_v)$.
...
\item and $P \in\Omega_v\subseteq T(L_v) \cong T(L_w)$ via one such $K_v$-algebra morphism
(this makes sense as $\Omega_v$ is $\Gal(L_v/K_v)$-invariant).
\end{itemize}
\end{theorem}
