...
The regular-prime formalization supplies closed proofs for exponents $5$, $7$, $11$, and $13$,
and combines them with the cases $3$ and $4$ into the following finite-range result.

\begin{lemma}\label{FLT_small}\lean{FLT_small}\leanok
  Fermat's Last Theorem holds for every exponent $3\leq n\leq16$.
\end{lemma}

\begin{corollary}\label{FermatLastTheorem.of_small_and_p_ge_17}
  \lean{FermatLastTheorem.of_small_and_p_ge_17}\leanok
  To prove Fermat's Last Theorem it is enough to prove it for prime exponents $p\geq17$.
\end{corollary}
\begin{proof}\uses{FermatLastTheorem.of_p_ge_5, FLT_small}\leanok
  The previous reduction produces a prime $p\geq5$. If $p<17$, the result is
  \ref{FLT_small}; otherwise $p\geq17$.
\end{proof}
...
Now say $(a,b,c,p)$ is a Frey package with $p\geq17$.
...
\begin{theorem}[Mazur]
  ...
  \uses{FreyCurve}
  \leanok
  If $\rho$ is the mod $p$ Galois representation associated to a Frey package $(a,b,c,p)$
  with $p\geq17$, then
  $\rho$ is irreducible.
\end{theorem}
\begin{proof}
...
\begin{corollary}
  ...
  \leanok
  There is no Frey package whose exponent is at least $17$.
\end{corollary}
...
\end{corollary}
\begin{proof}
  \uses{FermatLastTheorem.of_small_and_p_ge_17, FreyPackage.false, FreyPackage.of_not_FermatLastTheorem_p_ge_5}
  Assume there is a there is a counterexample $a^n+b^n=c^n$.
  By Corollary \ref{FermatLastTheorem.of_small_and_p_ge_17} we may assume that there is also a
  counterexample $a^p+b^p=c^p$ with $p\geq 17$ and prime.
