...
to the statement that there is no prime $\ell\geq 17$ and hardly-ramified
...
be the number field corresponding to the kernel of~$\rho$.

\begin{theorem}[Potential modularity]
    \label{potential_modularity}
    \uses{moret-bailly,modularity_lifting_theorem}
    \notready
    There is some totally real field $F$ of even degree, Galois over $\Q$,
...
\begin{theorem}
    ...
    \uses{Skinner_Wiles_CFT_trick,local_galois_coh_dim_two,local_galois_coh_top_degree,
      ...
      Galois_representation_from_automorphic_representation_on_GL_2_form,
...
\phantomsection\label{hardly_ramified_lift}
We now use Khare--Wintenberger to lift $\rho$ to a potentially modular $\ell$-adic
Galois representation of conductor 2, and
\phantomsection\label{hardly_ramified_compatible_family}
put it into an $\ell$-adic family using
