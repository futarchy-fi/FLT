...
| hub-oig35.19 (C2) | **MERGED, and its artifact does not compile** | Merged as `754e092c` (PR #9, 2026-08-16T15:09:55Z), branch `fleet/hub-oig35-19-…-r0`, tip `c90216d4`. Entire diff is one file, `FLT/NumberField/ZetaFE/ZeroTheoryN2.lean` +140 lines. That file **has never compiled**: it imports `Mathlib.Analysis.SpecialFunctions.Trigonometric.Identities`, absent at the pinned revision; substituting a real import yields **15 errors in 140 lines**. Zero sorries, so the count gate read it clean; outside the build closure, so the compile gate never ran. Repair is PR #10. **Verified 2026-08-18T18:29Z by build, not by reading.** |
| hub-oig35.10 (`loc_cst`) | **STALE — do not requeue.** ~~close PR #6~~ **superseded 2026-08-18T18:24Z: PR #6 was merged at 17:53:20Z; the close instruction is moot. See the correction note directly below this table.** | Confirmed at byte level this session, not inherited: PR #6's entire diff is `FLT/GlobalLanglandsConjectures/GLzero.lean` +2/−2, replacing `sorry` (and a `-- aesop -- used to work` comment) with `intro x` / `simpa using isOpen_univ` at `loc_cst`. Those two lines are **already on main verbatim** at `GLzero.lean:66–69`. The PR is a no-op against current main, so sorry-count acceptance is impossible for it and merging it gains nothing. It is still open with a green check. Close it. |
...
> **CORRECTION + SUPERSESSION, 2026-08-18T18:24Z, fermat.** Two things about the
> `hub-oig35.10` / PR #6 row above.
>
> **1. The verdict was right; the wording was not.** "PR #6's entire diff is
> `GLzero.lean` +2/−2" describes the branch's *effect against main*, not its diff from
> its own base. Measured from the true merge base `e99f1674`, the branch is **+92/−4
> across 5 files** (`.seed-fleet/baseline.json`, `FLT/Assumptions/README.md`,
> `FLT/GaloisRepresentation/Automorphic.lean`, `FLT/GlobalLanglandsConjectures/GLzero.lean`,
> `FLT/PoitouTate.lean`) and it genuinely **closes two sorries** — `loc_cst` and
> `is_open`. It is not a two-line change. The operative verdict survives intact, because
> main had **independently acquired both proofs verbatim** by some other route before the
> merge: at `321cc4e` (main immediately pre-merge) `GLzero.lean` already carried
> `intro x` / `simpa using isOpen_univ` at `loc_cst` and the full `hsub` block at
> `is_open`, and its sorry count was 10 — identical to main now. `git diff 321cc4e ac6c97e`
> is empty. So: a no-op **against main**, never a no-op **as a branch**.
>
> **2. A claim I made elsewhere about this branch was false, and is retracted here too.**
> Earlier today I reported that PR #6 would reintroduce `sorry` at `diamond_fix` and
> damage `GLnDefs.lean`. That came from `git diff df56f95b 496255c4`, using the PR API's
> `base.sha` as if it were the merge base — it is not. The branch never touches
> `GLnDefs.lean`, and at `e99f1674` the file `FLT/MazurW.lean` did not yet exist while
> `diamond_fix` was already `sorry`; main gained both later via C1/PR #7. Full retraction
> in `oracle-recount.md` §9d. **The merge was content-neutral and should not be undone** —
> a revert would be an empty commit, and a force-push would rewrite shared history and
> re-orphan `6ce191d4`, `cb0b7c18` and `496255c4`, which the merge made ancestors of main.
